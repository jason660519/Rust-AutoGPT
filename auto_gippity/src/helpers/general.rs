...
const CODE_TEMPLATE_PATH: &str =
    r"C:\Users\a0922\Desktop\rust\rust_autogpt\web_template\src\code_template.rs";
const EXEC_MAIN_PATH: &str = r"C:\Users\a0922\Desktop\rust\rust_autogpt\web_template\src\main.rs";
const API_SCHEMA_PATH: &str =
    r"C:\Users\a0922\Desktop\rust\rust_autogpt\auto_gippity\schemas\api_schema.json";
...
pub async fn ai_task_request(
    ...
    // Extend AI function
    let extended_msg: Message = extend_ai_function(function_pass, &msg_contect);

    // Print current status
    PrintCommand::AICall.print_agent_message(agent_position, agent_operation);

    // Get LLM response
    let _llm_response_res: Result<String, Box<dyn std::error::Error + Send>> =
        call_gpt(vec![extended_msg.clone()]).await;

    // Return Success or try again
    match _llm_response_res {
...
// Peform call to LLM GPT -Decoded
pub async fn ai_task_request_decoded<T: DeserializeOwned>(
    ...
    let llm_response: String =
        ai_task_request(msg_contect, agent_position, agent_operation, function_pass).await;
...
pub fn save_api_endponts(api_endpoints: &String) {
